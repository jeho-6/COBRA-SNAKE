...
int ret_num_cmd = 0;  /* 保存ATK-MW8266D AT指令返回值 */
uint8_t* ret_num_rx = NULL;   /* 保存ATK-MW8266D UART接收数据返回值 */
int time = 0;  /* 用于ATK-MW8266D AT指令等待超时 */
int start_flag = 0;  /* ATK-MW8266D AT指令开始标志位 */
char* frame_ret = NULL;  /* 保存ATK-MW8266D UART接收数据返回值 */
/**
...
// {
//     if (g_atk_uart_state != ATK_UART_WORKING) {
//         printf("模块未ready，拒绝发送AT命令。\r\n");
...
uint8_t atk_mw8266d_send_at_cmd(char *cmd, char *ack, uint32_t timeout)
{
    uint8_t *ret = NULL;
    time = timeout;
    start_flag = 1;  /* 设置ATK-MW8266D AT指令开始标志位 */
    atk_mw8266d_uart_rx_restart();
    atk_mw8266d_uart_printf("%s\r\n", cmd);
    ...
    else
    {
        ...
        start_flag = 2;  /* 清除ATK-MW8266D AT指令开始标志位 */
        return ATK_MW8266D_ETIMEOUT;
    }
}
...
//     while (timeout--) {
//         uint8_t *frame = atk_mw8266d_uart_rx_get_frame();
//         frame_ret = (char*)frame; // 保存返回值
//         if (frame != NULL) {
//             if (strstr((char *)frame, "ready") != NULL) {
//                 printf("模块已ready，进入工作状态。\r\n");
//                 g_atk_uart_state = ATK_UART_WORKING;
//                 atk_mw8266d_uart_rx_restart();
//                 break;
//             } else {
//                 printf("丢弃启动信息：%s\r\n", frame);
